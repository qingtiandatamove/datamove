...
### 5.8 迁移模板市场 - 模板中心
- **它是什么**: 把「老手才知道该怎么配的参数组合」沉淀成场景模板，新人选场景、填三个字段就能建出一个能跑的任务
- **入口**: 顶部「数据集成中心」→「模板市场」(`/sync/template`)，排在「同步任务」前面 —— 先选模板建任务，再进任务列表看
- **内置模板** (`TaskTemplateRegistry`，纯代码注册，不用建表、不用改前端):

| 模板 | 适用 | 预置参数要点 |
| --- | --- | --- |
| MySQL → MySQL 全量迁移 | 搬历史数据 / 首次建库 | FULL + ID 游标，批次 2000，追加写入，手动触发 |
| 全量 + 增量(不停机迁移) | 业务不停机迁移 / 双写过渡 | INCR + Canal 订阅，DML 全收，位点持久化可续 |
| 业务库 → 测试库 | 给测试环境灌数据 | 批次降到 500 限流，**覆盖写入** |
| 业务库 → 数仓(每日增量) | 报表库 T+1 同步 | TIME 游标 + CRON `0 0 2 * * ?` 每天 02:00 |
| 大表分批迁移 | 千万行以上大表 | 8 分片并行 + 批次 5000，缩短迁移窗口 |
| 敏感字段脱敏迁移 | 生产数据外发 / 给外包供数 | 排除 password/id_card/phone 等常见敏感列 |

- **怎么用**: 点「详情」看预置参数 + 使用提示 + 限制说明 → 点「一键套用」→ 只填**源数据源 / 目标数据源 / 同步表**，
  以及模板真正需要的几个字段（ID 游标要主键字段、TIME 游标要时间字段、定时模板要 CRON、增量模板要 Canal 地址）；
  其余参数（批次 / 分片 / 覆盖 / 忽略字段 / DML 类型）由模板预置，任务名留空自动生成
- **套用后**: 创建的就是一个**普通 `sync_task`**，不做任何特殊标记 —— 任务页可继续改任何参数，监控 / 日志 / 校验 / 告警 / 权限全部照常生效；
  任务备注会写明「由模板『XXX』创建」，勾选「自动配对字段」还会按同名列生成字段映射（失败也不影响任务，可手动配）
- **定位**: 模板是「经验参数」不是黑盒，套用完想怎么改就怎么改；源库与目标库不能是同一个，表名做合法性校验
