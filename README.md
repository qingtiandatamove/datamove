...
| 在线 SQL 工作台 | 任意数据源直接写 SQL 跑（多语句分号分隔），**单语句 30s 超时**防卡死，支持 EXPLAIN（含 ANALYZE 二次确认）+ SQL 收藏 + 操作日志全程留痕 |
...
采用 **`feature → dev → test → master`** 的单向流向，每层只接收上一层的合并，避免出现"跳过测试直接上线"：

| 分支 | 职责 | 从哪来 | 合到哪去 |
| --- | --- | --- | --- |
| `master` | **生产可用版本**，随时可发布 | ← `test` | 只接收 `test` 与 `hotfix/*` |
| `test` | 测试环境验证，通过后才允许上生产 | ← `dev` | 验证通过 → `master` |
| `dev` | **日常开发集成分支（默认工作分支）** | ← `feature/*` | 自测通过 → `test` |
| `feature/*` | 单个功能 / 缺陷修复开发 | ← `dev` | 开发完成 → `dev` |
| `hotfix/*` | **线上紧急修复**，走最短路径 | ← `master` | 修完 → `master`，并回合并 `dev` |

**流向示意**

```
feature/xxx ──► dev ──► test ──► master          (常规功能)
                 ▲                   │
                 └──── hotfix/xxx ◄──┘           (紧急修复, 修完回合并 dev)
```

**约定**

- 所有开发在 `feature/*` 上做，**不直接 push 到 `dev` / `test` / `master`**
- 合并一律走 Pull Request，PR 目标分支按上表流向选择
- `master` 上每个提交都可发布；打 tag 即发版
- `hotfix/*` 修完必须**回合并到 `dev`**，否则下次发版会把修复弄丢

**把 `dev` 设为仓库默认分支**（Gitee 需在网页操作，命令行改不了）

> 仓库主页 → 「管理」 → 「仓库设置」 → 「基础设置」 → 「默认分支」 → 选择 `dev` → 保存

设置后：新克隆的仓库默认检出 `dev`，网页上创建 PR 的目标分支也默认是 `dev`。
